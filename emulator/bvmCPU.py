...
class CPU:
    ...
    def handleJumps(self, dest):
        if dest == 0x0C:
            ...
            self.ram[0x10 + self.sp * 3 - 3] = get_bits(
                pc, range(0, 4)
            )  # self.ram[0x0d]
            self.ram[0x10 + self.sp * 3 - 2] = get_bits(
                pc, range(4, 8)
            )  # self.ram[0x0e]
            self.ram[0x10 + self.sp * 3 - 1] = get_bits(
                pc, range(8, 12)
            )  # self.ram[0x0f]
            # Set the PC
            jsr = self.ram[0x0C]
    ...
    def ADC(self, args):
        ...
        if res == 0:
            ...
            self.Z = 0
        if (signed(a, 4) + signed(b, 4) + self.C > 7) or (
            signed(a, 4) + signed(b, 4) + self.C < -8
        ):
            self.V = 1
        else:
            self.V = 0
    ...
    def SBB(self, args):
        ...
        if res == 0:
            ...
            self.Z = 0
        if (signed(a, 4) - signed(b, 4) - int(self.C == 0) > 7) or (
            signed(a, 4) - signed(b, 4) - int(self.C == 0) < -8
        ):
            self.V = 1
        else:
            self.V = 0
